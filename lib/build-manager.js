...
export default class BuildManager {
  ...
  startBuild(id, io, onId) {
    ...
    .then(({project, data, userConfig}) => {
      try {
        return this.runBuild(io, {project, data, userConfig})
          .catch(error => {
            ...
            return this.dao.putBuild(data).then(() => {
              ...
              return {build: data, project: project}
            })
          })
      } catch (error) {
        handleError(error, data)
  ...
  runBuild(io, {project, data, userConfig}) {
    ...
    Object.keys(project.plugins).forEach(id => {
      ...
      if (!plugin) {
        throw new ConfigError(`Plugin ${id} is not installed`, 'plugins', 'Add it to your config.json in the base of the jaeger installation directory.')
      }
...
function wrapPluginStep(id, runner) {
  return function (step, fn, prepend) {
    return runner.use(step, function (builder, ctx, io) {
      console.log(builder.constructor.name, builder.pluginProxy + '')
