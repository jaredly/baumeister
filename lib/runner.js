...
export default class Runner {
  ...
  run() {
    const Build = buildPlugins[this.project.build.id]
    this.build = new Build(this.project, this.id, this.project.build.config)
    this.stage = STAGES[0]
    this.ctx = {}
  ...
  runStage() {
    ...
    return this.stagePromises(this.stage)
      .then(() => this.runNextStage())
  }
  ...
  stagePromises(stage) {
    ...
    if (this.build[this.stage]) {
      p = p.then(() => this.build[this.stage]())
    }
    return p
  }
